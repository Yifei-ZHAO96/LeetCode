...
def logistic_loss(X, y, W, b, alpha=0, norm=None):
    ...
    loss = -1 / num_data * \
        np.sum(y * np.log(y_hat) + (1 - y) * np.log(1 - y_hat)) + norm_term
...
if __name__ == '__main__':
    ...
    X, y = make_classification(
        n_samples=1000, n_features=10, n_informative=8, n_clusters_per_class=2)
    y = y.reshape(-1, 1)
    ...
    loss_list, params, grads = train(
        X_train, y_train, 0.001, 10000, alpha=0.1, norm='l2')
    print("params:", params)
